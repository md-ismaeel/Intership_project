...
export default function ColorCard({ image, color, isSelected = false, onSelect, title = "Theme preview" }: ColorCardProps) {
    ...
    return (
        <div
            ...
            className={`relative w-72 flex flex-col justify-center items-center space-y-3 group rounded-sm transition-all duration-300 cursor-pointer ${isSelected ? "ring-2 ring-offset-2" : "hover:bg-gray-50"}`}
            style={{
                backgroundColor: isSelected ? `${color}` : "transparent",
                borderColor: isSelected ? color : "transparent",
            }}
        >
            <div className="relative h-64 w-64 overflow-hidden rounded-lg shadow-lg transition-all duration-300 hover:shadow-xl">
                <img
                    src={image}
                    alt={title}
